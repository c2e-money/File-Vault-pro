...
// Configure Multer Disk Storage (Temporary folder for Google Drive upload processing)
...
// Google Drive Authentication Setup using credentials.json
...
const FOLDER_ID = '1CM0Vq7SXrfaZsTHr4u8ufxqA2RLHT3ER';
...
// Upload File (Single or Multiple with Google Drive Sync)
      ...
      for (const file of reqFiles) {
        ...
        if (drive) {
          try {
            ...
            // Delete temporary local file after Google Drive upload
            if (fs.existsSync(filePath)) {
              fs.unlinkSync(filePath);
            }
          } catch (driveErr) {
            console.error('Google Drive sync failed, falling back to local storage:', driveErr);
          }
        }
        ...
        const newFile: FileItem = {
          ...
          filePath: storageType === 'drive' ? webViewLink : file.path,
          fileSize: file.size,
          mimeType: file.mimetype || 'application/octet-stream',
